...
class Minerals_relation_details extends CI_Controller {
	...
	public function Acquisitions() {
		try {
			...
			if ($fieldName != null && $fieldName == "EchangeId") {
				...
				$this -> db -> where('EchangeId', $fieldValue);
				$query = $this -> db -> get('jointureEchangesAcquisitions');
				$ids = array();
				foreach ($query->result() as $row) {
					array_push($ids, $row -> AcquisitionId);
				}
				if (count($ids) > 0) {
					foreach ($ids as $value) {
						$crud -> or_where('AcquisitionId', $value);
					}
				} else {
	...
	public function Echanges() {
		try {
			...
			$crud -> set_relation('PersonneId', 'Personnes', '{PersonneMoraleNom} {PersonneNom} {PersonnePrenom}') -> set_relation_n_n('Sorties', 'JointureEchangesSorties', 'SortiesCollection', 'EchangeId', 'SortieId', '{SortieDate} {SortiePrecision}') -> set_relation_n_n('Acquisitions', 'jointureEchangesAcquisitions', 'Acquisitions', 'EchangeId', 'AcquisitionId', '{AcquisitionDate} - {Acquisitions.AcquisitionId}');
	...
	public function Manifestations() {
		try {
			$crud = new grocery_CRUD();
			$crud -> set_table('Manifestations') -> set_relation('CommuneId', 'Communes', 'CommuneNCCENR');
	...
	public function Regions() {
		try {
			$crud = new grocery_CRUD();
			$crud -> set_table('Regions') -> set_relation('EtatId', 'Etats', 'EtatNom');
	...
	public function sites() {
		try {
			...
			$crud -> set_table('Sites') -> set_subject('Site') -> display_as('CommuneId', 'Commune') -> set_relation('CommuneId', 'Communes', 'CommuneNCCENR');

			$crud -> set_parent_add_form('CommuneId', $this -> basePath . 'Communes/add?add=true');
			$crud -> set_parent_add_form_label_field("SiteNom");
			$crud -> set_relation_n_n('Mineraux', 'jointureSitesMineraux', 'Mineraux', 'SiteId', 'MineralId', 'MineralNom') -> set_relation_n_n('Fiches_BRGM', 'SitesJoinBRGMExploitations', 'BRGMExploitations', 'SiteId', 'NumFiche', 'NomExploitation');
	...
	public function SortiesCollection() {
		try {
			...
			if ($fieldName != null && $fieldName == "EchangeId") {
				$this -> db -> select('SortieId');
				$this -> db -> where('EchangeId', $fieldValue);
				$query = $this -> db -> get('JointureEchangesSorties');
				$ids = array();
				foreach ($query->result() as $row) {
					array_push($ids, $row -> SortieId);
				}
				if (count($ids) > 0) {
					foreach ($ids as $value) {
						$crud -> or_where('SortiesCollection.SortieId', $value);
					}
				} else {
	...
	public function SortiesSurTerrain() {
		try {
			...
			$crud -> set_table('SortiesSurTerrain') -> set_relation('SiteId', 'Sites', 'SiteNom') -> set_relation('PersonneId', 'Personnes', '{PersonneMoraleNom} {PersonnePrenom} {PersonneNom}');
